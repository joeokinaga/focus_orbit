...
| sqlite3 | **2.9.4（完全固定・lock実値で確定済み 2026-07-07）** | AD-3/P2-T1。`SqliteException` 型判定（UNIQUE違反→AlreadyApplied）。**注意: drift_dev 2.31 の床は ^2.4.6 — 2.4.0固定は解決不能（⑮で実証）** |
| sensors_plus | ^7.0.0 | AD-4 |
| amplify_flutter / api / auth_cognito | **一時除去（pubspecでコメントアウト中・2026-07-07確定）** | AD-5 の設計は不変。除去理由: 推移依存 connectivity_plus 7.1+ が Xcode 26 必須（⑬）で、Phase 3 のソロMVPは Amplify を1行も実行しないため（§13）。**再導入 = P4-T0 環境ゲート通過後、完全固定表記で** |
| device_info_plus | **11.0.0（完全固定）** | P3 実機検証中の iPhone 接続問題の回避としてダウングレード固定（2026-07-07）。導入経緯の詳細は未登記 — **P4-T0 で要否を棚卸しし、不要なら除去、必要なら本行に用途を追記すること** |
...
4. `flutter test` — 期待出力: `+135: All tests passed!`

状態: **DONE（2026-07-07・開発機ローカルで全4手順 GREEN 確認済み。135 = P1 の 112 + P2 の drift 18 + P3 の結線 smoke 1 + 請求リトライ 4）**。
...
├── main_sensor_debug.dart                    # センサー実機検証用エントリ(第二CR)
├── main_e2e.dart                             # E2E用エントリ(第三CR): 報酬x100のみ差替(P3-T2足場)
├── app/
...
docs/
└── E2E_CHECKLIST.md                          # P3-T2 実機手順書（Phase 3 最終ゲート）
test/
...
実行検証（開発機・全合格・2026-07-07更新）: `flutter analyze` 0件 / `flutter test` **135件 GREEN**（stance_detector 22 / session_transition 71=静的14+表駆動57 / economy_ledger 19 / drift_preferences_repository 18 / 結線smoke 1 / 請求リトライ 4=session_controller_claim）。
...
実機E2E（P3-T2・2026-07-07）: docs/E2E_CHECKLIST.md 全節（導線A/状態機械B/報酬べき等性C/ショップ経済D/後片付けE）を実機で実施し**全項目問題なし**（オーナー実施・報告による）。LANDMINE ③ は基準端末で問題なく通過 — ただし「端末により要検証」の性質上、新規端末サポート時の再検証ルールとして存続。
...
- Phase 3 追補（2026-07-07 確定分）= P3-T1 **DONE**（実機初回フレーム到達を戻るボタン検証で確認 — 完了基準4点すべて充足）/ P3-T1b **DONE**（A1導線表示・A2遷移/復帰を実機確認。A3=セッション中の非表示のみ E2E A3 で最終化）/ P3-UX0 戻るボタン **DONE**（analyze 0・実機正常）/ iOSビルド障害の適用解 = **amplify_* 一時除去（第0案）で確定**（§1・⑬に反映済み）/ device_info_plus 11.0.0 固定（§1）。
- P3-T2 実機E2E = **DONE（2026-07-07・全項目PASS）**。P3-T4 = **DONE（本更新）**。
- **★ Phase 3 クローズ宣言（2026-07-07）:** DI結線・ソロMVP統合・135テスト・実機E2Eの全ゲート通過。ソロMVPは実機で動作する完成状態。
- **IN-FLIGHT:** なし。
- **E2E足場（P3-T2用・releaseビルド非到達）:** debug限定1分プリセット(focus_view) / main_e2e.dart(報酬x100) / ShopController.debugSeedCoins(台帳を通る正規earn取引・'debug-seed:{epoch_us}'キー。**フラグ書換の裏口は設計上存在しない — 解放状態はD14により台帳の射影であり、書き換え対象のフラグ自体が無い**)。
- **NEXT（依存順）:** (1) P3-UX1 ショップ刷新 = 提示済み構造案（ネオン=装備モチーフの動的発光・FoGlassPanel/WalletDashboard/MotifShowcaseCard・純描画props設計）に基づく実装。着手条件は解除済み（ゲート通過） (2) P4-T0 環境ゲート = Xcode 26系更新（⑬）＋amplify_* 再導入（完全固定表記）＋device_info_plus 棚卸し (3) P4-T1〜T5 = §6契約のAppSync実装（詳細グラフは §11 旧記録および P4計画を参照）。
...
| ⑬ | connectivity_plus 7.1.0+（amplify_api 2.12+ の推移依存）は iOS 26 SDK の `NWPath.isUltraConstrained` を使用し **Xcode 26.1.1+ が必須**。Xcode 16.4/iOS 18 SDK ではコンパイル不能。キャッシュ削除・sedパッチは無効（pub cache repair で原本復元）。適用解 = **amplify_* の一時除去（第0案・2026-07-07確定）**。amplify 再導入/更新の前提条件 = Xcode 26 系（P4-T0 環境ゲート） | **P4拘束** | **P3** | pubspec.lock のコミット＋P4-T0 ゲート |
